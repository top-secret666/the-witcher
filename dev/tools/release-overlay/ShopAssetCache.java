...
public final class ShopAssetCache implements ShopRuntimeAssets, ShopOverlayAssets {
    ...
    final BufferedImage btnBuyHover;
    ...
    /** Брифинг: портрет только когда говорит Геральт. */
    final BufferedImage geraltSpeakScaled;
    final BufferedImage dukeScaled;
    final BufferedImage dukeLaughScaled;
    /** Брифинг: новый портрет только когда герцог говорит (без обработки альфы). */
    final BufferedImage dukeSpeakScaled;
    ...
    final BufferedImage inventoryPanelBg;
    ...
    private ShopAssetCache() {
        ...
        btnBuyHover = loadSized(UI + "shop_btn_buy_hover.png", btnW, btnH,
            BASE + "ui/shop_btn_buy_hover.png", false);
        ...
        geraltSpeakScaled = loadPortraitAsIs("geralt_portrait_speak.png", charH);
        // Лавка / до брифинга — старые ассеты как было.
        dukeScaled = loadPortrait("duke_portrait_shop.png", charH);
        dukeLaughScaled = loadPortrait("duke_portrait_fun_shop.png", charH);
        // Как в resources: уже вырезанный PNG, без кропа и без подложки.
        dukeSpeakScaled = loadPortraitAsIs("duke_portrait_speak.png", charH);
        ...
        inventoryPanelBg = loadSized(
            UI + "inventory_panel_bg.png",
            ShopViewConstants.INVENTORY_PANEL_W,
            ShopViewConstants.INVENTORY_PANEL_H,
            BASE + "ui/inventory_panel_bg.png",
            false);
    ...
    /** Speaking-портрет: только масштаб, альфа/вырезка из файла не трогаем. */
    private BufferedImage loadPortraitAsIs(String name, int targetH) {
        // Только оригинал lavka/ — не 1x bake (там раньше подсовывали чёрный RGB).
        BufferedImage src = load(BASE + name);
        if (src == null) {
            src = load(BAKED + name);
        }
        if (src == null) {
        ...
        int w = Math.max(1, Math.round(src.getWidth() * ((float) targetH / src.getHeight())));
    ...
    public BufferedImage inventoryPanelBg() {
        return inventoryPanelBg;
    }
  ...
  /** Speak-портрет Геральта — только реплики в брифинге. */
  public BufferedImage geraltSpeakPortrait() {
    return geraltSpeakScaled != null ? geraltSpeakScaled : geraltScaled;
  }
  ...
  /** Новый ассет только для реплик герцога в брифинге. */
  public BufferedImage dukeSpeakPortrait() {
    return dukeSpeakScaled;
  }
