...
public final class ShopSwingView implements ShopView {
    ...
    public void renderTextOverlay(Graphics2D g, int mouseX, int mouseY) {
        ...
        try {
            ...
            if (walletScene || purchaseScene || battleCardScene) {
                ...
                // Курсор рисует Chapter1SwingView поверх кнопки паузы.
                return;
            }
            ...
            // Курсор — в Chapter1SwingView после кнопки паузы.
        } finally {
    ...
    private void drawBattleCardRevealBag(Graphics2D g, ShopLayout layout) {
        ...
        float openT = bagOpenProgress(
            ...
            BATTLE_CARD_APPEAR_TICKS,
            BATTLE_CARD_FLY_TICKS,
            BATTLE_CARD_FADE_TICKS,
            BATTLE_CARD_CLOSE_TICKS);
    ...
    private void drawBattleCardFlying(Graphics2D g, int sw, int sh, ShopLayout layout) {
        ...
        int appearEnd = BATTLE_CARD_APPEAR_TICKS;
        int flyEnd = appearEnd + BATTLE_CARD_FLY_TICKS;
        int fadeEnd = flyEnd + BATTLE_CARD_FADE_TICKS;
        ...
        if (ticks <= appearEnd) {
            ...
            flyT = Math.min(1f, (ticks - appearEnd) / (float) Math.max(1, BATTLE_CARD_FLY_TICKS));
            ...
            py = cy - pw / 2f;
            alpha = 1f;
            if (ticks > flyEnd && BATTLE_CARD_FADE_TICKS > 0) {
                tuckT = smoothstep((ticks - flyEnd) / (float) BATTLE_CARD_FADE_TICKS);
                pw = minSize;
                px = bagCenterX - pw / 2f;
                py = bagCenterY - pw / 2f;
                alpha = Math.max(0f, 1f - tuckT);
            } else if (ticks > flyEnd) {
                return;
            }
        }
