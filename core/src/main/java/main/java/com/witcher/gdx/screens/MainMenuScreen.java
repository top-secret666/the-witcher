...
public class MainMenuScreen implements Screen {
    ...
    public void render(float delta) {
        ...
        controller.update(VW, VH, mouseX, mouseY, Gdx.input.isButtonJustPressed(Input.Buttons.LEFT));
    ...
    /** Тёплые парящие искры — порт Swing {@code drawAtmosphere}. */
    private void drawEmbers() {
        ...
        for (float[] e : controller.embers()) {
            ...
            float topX = e[0];
            float topY = e[1];
            float cx = topX + radius;
            float cy = C.centerY(topY, radius * 2f);

            if (sz > 1.2f && alpha > 0.3f) {
                float glowR = radius * 3f;
                shapes.setColor(1f, 0.63f, 0.24f, alpha * 0.15f);
                shapes.circle(cx, cy, glowR);
            }
